...
import { selectTheme, setTheme } from "../../slices/editorSlice";
import { THEMES } from "../constants/Monaco.constants";
...
import { LuCircleOff, LuGithub } from "react-icons/lu";
import { FaLaptop } from "react-icons/fa6";
import { CiCloudOn } from "react-icons/ci";

const THEME_ICONS: Record<string, React.ReactNode> = {
    "vs-dark": <FiMoon className="size-4" />,
    "vs-light": <MdOutlineWbSunny className="size-4" />,
    "github-dark": <LuGithub className="size-4" />,
    monokai: <FaLaptop className="size-4" />,
    "solarized-dark": <CiCloudOn className="size-4" />,
};
...
export default function ThemeSelector() {
    ...
    const handleThemeChange = (newTheme: string) => {
        dispatch(setTheme(newTheme)); // localStorage write is inside the slice
        setIsOpen(false);
    };
